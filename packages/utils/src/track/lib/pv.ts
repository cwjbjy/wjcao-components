 ...
 * option 请求参数
 */
function tracePageView({ currentUrl, delay }: OptionType) {
  setTimeout(() => {
    emit({
      eventType: 'pv',
...
function init({ pv }: InitOption) {
  ...
  window.history.pushState = function () {
    historyPushState.apply(window.history, arguments as any);
    tracePageView({ currentUrl, delay: Date.now() - visitTime });
    currentUrl = window.location.href;
  };

  window.history.replaceState = function () {
    historyReplaceState.apply(window.history, arguments as any);
    tracePageView({ currentUrl, delay: Date.now() - visitTime });
    currentUrl = window.location.href;
  };
